...
// Norm
template <class T, size_t R>
constexpr real_t normsq(const TRowVector<T, R> &rowvector) {
    real_t sumsq = 0;
    for (size_t r = 0; r < R; ++r)
...
template <class T, size_t R>
constexpr real_t norm(const TRowVector<T, R> &rowvector) {
...
template <class T, size_t C>
constexpr real_t norm(const TColVector<T, C> &colvector) {
    return sqrt(normsq(colvector));
}

template <class T, size_t N>
constexpr real_t normsq(const Array<T, N> &vector) {
    real_t sumsq = 0;
    for (size_t i = 0; i < N; ++i)
...
template <class T, size_t N>
constexpr real_t norm(const Array<T, N> &vector) {
