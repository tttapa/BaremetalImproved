...
Quaternion MadgwickAHRSUpdate(Quaternion orientation, IMUMeasurement imu) {
    ...
    qDot1 = 0.5f * (-orientation[1] * imu.gx - orientation[2] * imu.gy -
                    orientation[3] * imu.gz);
    qDot2 = 0.5f * (orientation[0] * imu.gx + orientation[2] * imu.gz -
                    orientation[3] * imu.gy);
    qDot3 = 0.5f * (orientation[0] * imu.gy - orientation[1] * imu.gz +
                    orientation[3] * imu.gx);
    qDot4 = 0.5f * (orientation[0] * imu.gz + orientation[1] * imu.gy -
                    orientation[2] * imu.gx);
    ...
    if (!((imu.ax == 0.0f) && (imu.ay == 0.0f) && (imu.az == 0.0f))) {
        ...
        recipNorm =
            invSqrt(imu.ax * imu.ax + imu.ay * imu.ay + imu.az * imu.az);
        imu.ax *= recipNorm;
        imu.ay *= recipNorm;
        imu.az *= recipNorm;
        ...
        s1 = _4q1 * q3q3 - _2q3 * imu.ax + 4.0f * q0q0 * orientation[1] -
             _2q0 * imu.ay - _4q1 + _8q1 * q1q1 + _8q1 * q2q2 + _4q1 * imu.az;
        s2 = 4.0f * q0q0 * orientation[2] + _2q0 * imu.ax + _4q2 * q3q3 -
             _2q3 * imu.ay - _4q2 + _8q2 * q1q1 + _8q2 * q2q2 + _4q2 * imu.az;
        s3 = 4.0f * q1q1 * orientation[3] - _2q1 * imu.ax +
             4.0f * q2q2 * orientation[3] - _2q2 * imu.ay;
