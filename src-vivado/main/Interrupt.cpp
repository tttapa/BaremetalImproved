...
static const int INTC_DEVICE_ID = XPAR_PS7_SCUGIC_0_DEVICE_ID;
...
int setupIICInterruptSystem() {
	...
	static int IIC_INT_VEC_ID = XPAR_XIICPS_0_INTR;
	 ...
	 */
	int status = XScuGic_Connect(&InterruptController, IIC_INT_VEC_ID,(Xil_InterruptHandler)XIicPs_MasterInterruptHandler,	(void *)&Iic0);
	if (status != XST_SUCCESS) {
...
int setupIMUInterruptSystem() {
	...
	static int GYR_INT_ID = XPAR_FABRIC_SYSTEM_CORE1_NIRQ_INTR;
     ...
     */
	int status = XScuGic_Connect(&InterruptController, GYR_INT_ID, (Xil_InterruptHandler)int_gyr, (void *)&InterruptController);
	if (status != XST_SUCCESS)
	{
...
unsigned char iicConfig(unsigned int DeviceIdPS, XIicPs* iic_ptr) {
	...
	*/
	...
	static int IIC_SCLK_RATE = 100010; // TODO: inconsistent with used clock rate

	// Set the IIC serial clock rate.
	status = XIicPs_SetSClk(&Iic0, 400000); // TODO: should use IIC_SCLK_RATE
	if (status != XST_SUCCESS) {
